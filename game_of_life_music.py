...
def call_dances(dances, arms, birth_ids, kill_ids, living_ids, dying_ids, first_birth_ids):
    t1 = threading.Thread(target=alive_dance, args=([birth_ids], dances, arms))
    t2 = threading.Thread(target=dead_dance, args=([kill_ids], dances, arms))
    t3 = threading.Thread(target=living_dance, args=([living_ids], dances, arms))
    t4 = threading.Thread(target=dying_dance, args=([dying_ids], dances, arms))
    t5 = threading.Thread(target=first_birth_dance, args=([first_birth_ids], dances, arms))
...
def test_without_arms():
    ...
    game.run_game_contagion(robots, 0, 6, 1)
    # first_robot = randrange(10)
