...
class Robot:
    ...
    def flock(self):
        ...
        new_angle = align - self.joint_0
        if (self.joint_0 + new_angle) > 360:
            print("/n over 360 /n")
            spill_over = new_angle - 360
            new_angle = self.joint_0 - spill_over

        new_angle = align
        self.joint_0_moves.append(new_angle)
    ...
    def define_dance(self):
        new_step = [0, 0, 0, 0, self.joint_0_moves[0], 0, 0]
        self.dance.append(new_step)
        new_time = [5, 5, 5, 5, 5, 5, 5]
        self.dance_t.append(new_time)
        for i in range(len(self.joint_0_moves)-1):
            new_step = [0, 0, 0, 0, (self.joint_0_moves[i+1] - self.joint_0_moves[i]), 0, 0]
            self.dance.append(new_step)
            new_time = [5, 5, 5, 5, 5, 5, 5]
            self.dance_t.append(new_time)
